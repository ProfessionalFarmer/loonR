...
#'
#' @examples
ClusterProfiler.OverRepresentationTest <-  function(gene, minGSSize = 10, 
  ...
  if(GO){
    ego <- enrichGO(gene          = loonR::id_mapping(gene, key="SYMBOL", column="ENTREZID") %>% pull(ENTREZID),
                    OrgDb         = org.Hs.eg.db,
                    ont           = "BP",
                    pAdjustMethod = "BH",
                    pvalueCutoff  = 0.05,
  ...
  if(KEGG){
    kk <- enrichKEGG(gene         = loonR::id_mapping(gene, key="SYMBOL", column="ENTREZID") %>% pull(ENTREZID),
                     organism     = 'hsa',
                     pAdjustMethod = "BH",
                     pvalueCutoff  = 0.05,
                     qvalueCutoff  = qvalue,
